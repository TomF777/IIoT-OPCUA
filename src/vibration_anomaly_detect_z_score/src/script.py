"""
Poll single vibration sensor (acceleration data) over OPC UA.
Calculate anomalies using z-score method and stores results in InfluxDB.
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    vib_accel_rms_x   = opc_node_children[3].get_value()
                    vib_accel_rms_y   = opc_node_children[4].get_value()
                    vib_accel_rms_z   = opc_node_children[5].get_value()
                    sensor_temperature = opc_node_children[6].get_value()
                    timestamp     = opc_node_children[7].get_value()
                except Exception as e:
                    ...
                    if measurement.get(str(sensor_name)) != timestamp:
                        ...
                        # root sum square to represent worst case for each axis
                        vib_total_rms = round(
                            float(
                                math.sqrt(
                                    vib_accel_rms_x ** 2
                        ...
                        self.sensor_analytics.check_if_anomaly(vib_total_rms)
...
if __name__ == "__main__":
    ...
    # Configure conection with InfluxDB database
    try:
